...
export default function CreateDocButton() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (values.title === "") {
      setIsFetching(false);
      return toast.error("Please enter a title for your document.")
    ...
    if (!res?.ok) {
      ...
      return toast.error("our document was not created. Please try again.")
      /* toast({
    ...
    startTransition(() => {
      ...
      router.refresh();
      router.push(`/notes/${document.publicId}`);
    });
  }
  ...
  return (
    <>
      <Button
        ...
        variant="outline"
        className="h-8 w-full justify-start rounded-none hover:bg-gray-500"
      >
